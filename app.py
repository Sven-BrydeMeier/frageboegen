...
def page_settings():
    ...
    url = get_app_url()
    key = get_openai_key()
    col1.success(f"✅ APP_URL: {url}") if url else col1.warning("⚠️ APP_URL fehlt")
    col2.success(f"✅ OPENAI_API_KEY: {key[:8]}...") if key else col2.info("ℹ️ OPENAI_API_KEY fehlt")
    ...
    for i, e in enumerate(st.session_state.employees):
        ...
        st.session_state.employees[i]['name'] = col1.text_input("N", safe_get(e, 'name'), key=f"en_{i}", label_visibility="collapsed")
        st.session_state.employees[i]['email'] = col2.text_input("E", safe_get(e, 'email'), key=f"ee_{i}", label_visibility="collapsed")
        if col3.button("🗑️", key=f"ed_{i}"):
            st.session_state.employees.pop(i)
            st.rerun()
    
    col1, col2, col3 = st.columns([4, 4, 1])
    nn = col1.text_input("Name", key="nn", placeholder="Name")
    ne = col2.text_input("Email", key="ne", placeholder="email@ra-rhm.de")
    if col3.button("➕") and nn and ne:
    ...
    if 'invite' in st.query_params:
        ...
        if data:
            ...
            if fid in st.session_state.forms:
                ...
                st.error(f"❌ Formular {fid} nicht gefunden.")
                st.info(f"IDs: {list(st.session_state.forms.keys())[:10]}")
        else:
            st.error("❌ Ungültiger/abgelaufener Link")
    
    render_sidebar()
    
    pages = {'dashboard': page_dashboard, 'form': page_form, 'success': page_success,
             'invite': page_invite, 'invitations': page_invitations, 'settings': page_settings}
